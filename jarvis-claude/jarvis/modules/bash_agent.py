...
def _is_shell_expansion(token: str) -> bool:
    """True if the token would expand/glob at runtime ($VAR, ${..}, $(cmd),
    backticks, ~/* glob)."""
...
def _interpreter_inline_code(cmd: str) -> List[str]:
    """Scripts passed to interpreters via -c/-e (incl. combined shorts like
    -lc). Each snippet is screened separately against the hardline list."""
    toks = _tokens(cmd)
    snippets: List[str] = []
    prev_interpreter = False
    expect_code = False
    for tok in toks:
        if expect_code:
            if tok.startswith("-"):
        ...
        if base in _INTERPRETER_NAMES:
            prev_interpreter = True
            continue
        if (
            prev_interpreter
            and tok.startswith("-")
            and not tok.startswith("--")
            and ("c" in tok[1:] or "e" in tok[1:])
        ):
            expect_code = True
            continue
        prev_interpreter = False
...
def _is_hardline_blocked(cmd: str) -> Optional[str]:
    ...
    # Nested interpreter payloads hide from every pattern below:
    # sudo sh -c 'rm -rf /' — screen each -c/-e snippet recursively.
    for snippet in _interpreter_inline_code(cmd):
        hit = _is_hardline_blocked(snippet)
        if hit:
            return hit
    ...
    for desc, pat in (
        ...
        if hit and re.search(
            r"/dev/(sd|nvme|vd|mmcblk|hd|mapper/|disk/)", cmd_unquoted, re.IGNORECASE
        ):
            return hit
    ...
    regex_patterns = [
        ...
        (
            re.compile(r"systemctl\s+(stop|disable)\s+(sshd|multi-user|ufw|firewalld)"),
            "kill security service",
        ),
        ...
        (
            re.compile(r"\b(curl|wget|nc|ncat)\b[^|]*\|\s*(sudo\s+)?[\w./-]+"),
            "download-piped-executable",
        ),
...
_STARTUP_FILE_BASENAMES = frozenset(
    {
        ".bashrc",
        ".zshrc",
        ".zprofile",
...
def _touches_startup_file(cmd: str) -> bool:
    """True if ANY argument names a shell startup file — catches
    ``tee ~/.bashrc``, ``dd of=...``, ``cp x ~/.bashrc``, ``sed -i ...``."""
    return any(
        tok.rsplit("/", 1)[-1].lower() in _STARTUP_FILE_BASENAMES
...
_DANGEROUS_PATTERNS: List[tuple] = [
    ...
    (
        re.compile(r"\b(sudo\s+)?pip\s+install\b.*\bsudo\b", re.IGNORECASE),
        "sudo pip install",
    ),
    ...
    (
        re.compile(r"shutil\.rmtree|os\.(remove|unlink|system)\b", re.IGNORECASE),
        "python fs/system call",
    ),
    (
        re.compile(
            r">\s*~?/?(\.bashrc|\.zshrc|\.zprofile|\.profile|\.bash_profile)\b",
            re.IGNORECASE,
        ),
    ...
    (re.compile(r"\bcrontab\b(?!\s+-l\b)", re.IGNORECASE), "modify crontab"),
    (
...
def _detect_dangerous(cmd: str) -> List[str]:
    ...
    if _touches_startup_file(cmd):
        warnings.append("shell-startup file touched")
    return warnings
...
# $HOME/bin is on Debian's default PATH — executable persistence channel.
...
def check_approval(cmd: str, approval_mode: str = "auto") -> Optional[str]:
    ...
    if warnings:
        if approval_mode == "yolo":
            logger.warning(
                "⚠ YOLO executing dangerous command: %s — %s", cmd, "; ".join(warnings)
            )
            return None
