...
def _ensure_stub(name: str, attrs: dict | None = None):
    """Регистрирует фейковый модуль если оригинал не установлен."""
...
class TestVoskSTT:
    def _make_stt(self):
        with patch('jarvis.modules.stt.Model') as mock_model, \
...
class TestWhisperSTT:
    def test_whisper_instantiates_with_mocked_model(self):
        with patch('jarvis.modules.stt_whisper.WhisperModel') as mock_wm, \
    ...
    def test_whisper_has_recognize_method(self):
        with patch('jarvis.modules.stt_whisper.WhisperModel') as mock_wm, \
            ...
            mock_wm.return_value = MagicMock()
            from jarvis.modules.stt_whisper import WhisperSTT
            stt = WhisperSTT(model_size='tiny', sample_rate=16000,
                              device_name=None, use_vad=False)
...
class TestGTTSTempFile:
    ...
    def test_temp_file_cleaned_on_exception(self, monkeypatch, tmp_path):
        ...
        # файлов до и после).
        before = set(Path('/tmp/jarvis').glob('*.mp3')) if Path('/tmp/jarvis').exists() else set()
        gtts_inst.speak("test", play=False)
        after = set(Path('/tmp/jarvis').glob('*.mp3')) if Path('/tmp/jarvis').exists() else set()
        leaked = after - before
