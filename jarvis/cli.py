...
def cmd_presets(args):
    ...
    from jarvis.presets import (
        load_presets, save_presets, select_preset_or_create
    )
...
def cmd_voice(args):
    ...
    from jarvis.modules.tts import PiperTTS
...
Примеры:
  ...
  jarvis service install             # автозапуск при старте
    ...
    p_run.add_argument('-p', '--provider',
                       choices=['kiro', 'openrouter', 'anthropic', 'ollama'],
                       help='LLM провайдер (без меню)')
    p_run.add_argument('--preset', type=str, metavar='NAME',
                       help='Загрузить пресет провайдера')
    p_run.add_argument('--mode', choices=['classic', 'continuous'], default='classic',
                       help='Режим: classic (wake word) или continuous (без wake word)')
    p_run.add_argument('--wake-mode', choices=['classic', 'vad'], default='classic',
                       help='Метод детекции wake word')
    p_run.add_argument('--mute', action='store_true',
                       help='Старт в режиме тишины')
    p_run.add_argument('--dry-run', action='store_true',
                       help='Проверка конфига без микрофона')
    ...
    p_test.add_argument('-p', '--provider', choices=['kiro', 'openrouter', 'anthropic', 'ollama'])
    ...
    p_voice.add_argument('action', choices=['list', 'download'],
                         help='Действие')
    p_voice.add_argument('model', nargs='?', default='',
                         help='Название голоса (для download)')

    # ── dictation ──
    p_dict = sub.add_parser('dictation', help='Режим диктовки')
    p_dict.add_argument('-p', '--provider',
                        choices=['kiro', 'openrouter', 'anthropic', 'ollama'],
                        help='LLM провайдер (не исп. в диктовке)')
    p_dict.add_argument('--config', type=str, default='config.yaml',
                        help='Путь к конфигу')

    # ── service ──
    p_svc = sub.add_parser('service', help='Управление systemd-сервисом')
    p_svc.add_argument('action', choices=['install', 'remove', 'status'],
                       help='Действие')
