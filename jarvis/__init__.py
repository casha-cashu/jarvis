...
def setup_logging(config: dict, verbose: bool = False) -> None:
    ...
    log_level = (
        logging.DEBUG if verbose else getattr(logging, log_config.get("level", "INFO"))
    )
...
class Jarvis:
    ...
    def run(self) -> None:
        ...
        while self.running:
            try:
                if self.conversation.is_muted:
                    print("\r🔇 Режим тишины (скажи 'проснись')", end="", flush=True)
                ...
                if self.continuous:
                    ...
                    if not self.verbose:
                        print("\r💤 Жду 'джарвис'...", end="", flush=True)
